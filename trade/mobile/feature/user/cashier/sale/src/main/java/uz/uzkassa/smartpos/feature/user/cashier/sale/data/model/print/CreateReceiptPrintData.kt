...
internal class CreateReceiptPrintData(
    ...
    override fun getPrintContentData(context: Context) = ArrayList<PrintContentData>().apply {
        ...
        company.tin?.let {
            if (it.toString().length > 9) add(TextPair("JShShIR (ПИНФЛ):", it.toString()))
            else add(TextPair("STIR (ИНН):", it.toString()))
        }
        receipt.terminalSerialNumber?.let { add(TextPair("№ ККМ:", it)) }
        ...
        if (fiscalReceiptResult != null) {
            ...
            if (fiscalReceiptResult.receiptNumberInCurrentShift != null) {
                add(
                    TextPair(
        ...
        add(
            TextPair(
                "Chek turi:", when (receipt.status) {
                    ReceiptStatus.PAID -> "Sotuv"
                    ReceiptStatus.ADVANCE -> "Avans"
                    ReceiptStatus.CREDIT -> "Kredit"
                    else -> "Sotuv"
                }
            )
        )
...
//                }
        ...
        if (fiscalReceiptResult != null) {
            if (fiscalReceiptResult.receiptSeq != null) {
                add(
                    TextPair(
                        leftText = "Chek raqami:",
                        rightText = " №${fiscalReceiptResult.receiptSeq}"
                    )
                )
            }
            add(TextPair("UID: ", receipt.uid))
            if (receipt.status == ReceiptStatus.ADVANCE || receipt.status == ReceiptStatus.CREDIT) {
                if (receipt.originUid != null) {
                    add(TextPair("OriginUID: ", receipt.originUid!!))
                }
            }
...
//            }
    ...
    private fun Receipt.mapToPrintContentData(): List<PrintContentData> {
        ...
        if (totalDiscount > BigDecimal.ZERO) {
            list.add(TextPair("JAMI (ИТОГО)", totalCost.addWhitespace(), isBold = true))
            list.add(
                TextPair(
                    "CHEGIRMA (СКИДКА)",
                    totalDiscount.setScale(2, RoundingMode.DOWN).addWhitespace(),
                    isBold = true
                )
            )
        }
        ...
        list.add(
            TextPair(
                "HAMMASI (К ОПЛАТЕ)",
                (totalCost - totalDiscount).addWhitespace(),
                fontSize = 24,
                isBold = true
            )
        )
        ...
        if (totalCash > BigDecimal.ZERO)
            list.add(TextPair("   Naqd (Наличн.):", (totalPaid - totalCard).addWhitespace()))


        if (totalCard > BigDecimal.ZERO) {
            val totalApay: BigDecimal? =
                receiptPayments.find { it.type == ReceiptPayment.Type.APAY }?.amount
            if (totalApay == null) {
                list.add(TextPair("   Karta (Банк. карта):", totalCard.addWhitespace()))
            } else {
                list.add(TextPair("   A-pay :", totalApay.addWhitespace()))
                if (totalCard - totalApay > BigDecimal.ZERO) {
                    list.add(
                        TextPair(
                            "   Karta (Банк. карта):",
                            (totalCard - totalApay).addWhitespace()
                        )
                    )
                }
            }
        ...
        list.add(
            TextPair(
                ...
                totalVAT.setScale(2, RoundingMode.DOWN).addWhitespace(),
                isBold = true
            )
        )

        if (totalPaid > totalCost - totalDiscount)
            list.add(
                TextPair(
                    leftText = "QAYTIM (СДАЧА):",
                    rightText = (totalPaid - (totalCost - totalDiscount)).addWhitespace(),
                    fontSize = 24,
                    isBold = true
                )
            )
    ...
    private fun ReceiptDetail.mapToPrintContentData(
        ...
        if (discountAmount > BigDecimal.ZERO) {
            val discountPercentText =
                if (discountPercent == 100.0) "" else "- ${discountPercent.addWhitespace()}%"
